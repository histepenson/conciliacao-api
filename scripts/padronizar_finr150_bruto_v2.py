...
variante de layout com uma coluna extra "Natureza" (codigo numerico, sem a
descricao) inserida apos "Vencto Real" e SEM a coluna "DT Baixa" (diferente
de scripts/padronizar_finr150_bruto.py, que espera o layout original sem
essa coluna extra).

Layout esperado (header na 1a linha, 27 colunas):
Dados do fornecedor, Filial, Prefixo, No. Titulo, Parcela, Tipo,
Fornecedor, Loja, Dados da natureza, DT Emissao, Vencimento, Vencto Real,
Natureza, Vlr.Titulo, Saldo, Acrescimo, Decrescimo, Valores acessorios,
Abatimentos, Juros, Razao social, CPF/CNPJ, Saldo liquido, Atraso,
Historico, Portador, No do Cheque
...
(saldo em aberto), nao o Vlr.Titulo original nem o Saldo liquido. A coluna
"Natureza" de saida usa "Dados da natureza" (codigo + descricao), nao a
coluna numerica extra "Natureza" desse layout.

Uso:
    python scripts/padronizar_finr150_bruto_v2.py "C:\\...\\FINR150.xlsx"
"""
import argparse
import os
...
def carregar_padronizado(caminho: str) -> pd.DataFrame:
    ...
    # 10 Vencimento | 11 Vencto Real | 12 Natureza (codigo numerico) | 13 Vlr.Titulo
    # 14 Saldo | 15 Acrescimo | 16 Decrescimo | 17 Valores acessorios
    # 18 Abatimentos | 19 Juros | 20 Razao social | 21 CPF/CNPJ
    # 22 Saldo liquido | 23 Atraso | 24 Historico | 25 Portador | 26 No do Cheque

    bruto = bruto[bruto[1].notna()].copy()

    fornecedor = _s(bruto[6])
    loja = _s(bruto[7])
    razao_social = _s(bruto[20])
    ...
    saldo = pd.to_numeric(bruto[14], errors="coerce").fillna(0.0)
    atraso = pd.to_numeric(bruto[23], errors="coerce").fillna(0.0)
    ...
    saida = pd.DataFrame({
        ...
        "Vlr.juros ou permanencia": pd.to_numeric(bruto[19], errors="coerce").fillna(0.0),
        "Dias Atraso": atraso,
...
def main():
    parser = argparse.ArgumentParser(description="Padroniza FINR150.xlsx bruto (variante com coluna Natureza extra) no layout nativo de upload manual")

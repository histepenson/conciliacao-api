...
class Sn3Service:
    """Proxy para ZSN3API — Baixas de Ativo Fixo via SQL direto."""

    def __init__(self, protheus_base_url: str, user: str = "", password: str = "", tenant_id: str = "", rest_prefix: str = "rest"):
        self.endpoint = protheus_base_url.rstrip("/") + f"/{rest_prefix.strip('/')}/zsn3api/api/v1/sn3"
    ...
    async def buscar_pagina(self, params: dict[str, Any], *, client: httpx.AsyncClient | None = None) -> dict[str, Any]:
        ...
        async def _do(c: httpx.AsyncClient) -> dict[str, Any]:
            resp = await protheus_get(c, self.endpoint, params=query, headers=headers, logger=logger, operation=f"SN3 pagina {query['page']}")
            data = _decode_response(resp.content)
            total_pages = int(data.get("total_pages") or data.get("totalPages") or query["page"] or 1)
            logger.info("SN3 -> pagina %s/%s  pageSize=%s  endpoint=%s", query["page"], total_pages, query["pageSize"], self.endpoint)
            return data
    ...
    def _montar_query(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        query = {k: v for k, v in params.items() if k in _PARAMS_SN3 and v is not None}

...
class Ctbr400Service:
    ...
    async def buscar_pagina(self, params: dict[str, Any], *, client: httpx.AsyncClient | None = None) -> dict[str, Any]:
        ...
        async def _do(c: httpx.AsyncClient) -> dict[str, Any]:
            resp = await protheus_get(c, self.endpoint, params=query, headers=headers, logger=logger, operation=f"CTBR400 pagina {query['page']}")
            data = self._decode_response(resp.content)
            total_pages = int(data.get("total_pages") or data.get("totalPages") or query["page"] or 1)
            logger.info("CTBR400 -> pagina %s/%s  pageSize=%s  endpoint=%s  tenant=%s", query["page"], total_pages, query["pageSize"], self.endpoint, self.tenant_id)
            return data
    ...
    async def buscar_razao(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        async with protheus_async_client(auth=self.auth) as client:
            while has_more:
                ...
                logger.info(
                    "CTBR400 -> pagina %s/%s  pageSize=%s  endpoint=%s  tenant=%s",
                    current_page,
                    total_pages,
                    query["pageSize"],
                    self.endpoint,

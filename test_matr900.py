...
Uso:
    ...
    python test_matr900.py --via-backend          # chama localhost:8000 ao invés do Protheus direto
"""
...
def _paginar_tudo(base: str, path: str, params: dict, user: str, password: str, tenant_id: str) -> dict:
    """Busca todas as páginas e consolida."""
    page = 1
    all_linhas = []
    parametros = {}
    total_pages = 1
...
def main():
    ...
    parser.add_argument("--armazem",           default=None,        help="Armazém/Local (ex: 01)")
    parser.add_argument("--documento-por",     default="D",         help="D=Documento  S=Sequência")
    ...
    parser.add_argument("--tipo-custo",        default="1",         help="1=Médio  2=Reposição")
    parser.add_argument("--page-size",         default=500, type=int)
    parser.add_argument("--via-backend",       action="store_true", help="Chama localhost:8000 ao invés do Protheus direto")
    ...
    if args.via_backend:
        # Chama o backend local (que já cuida de paginação)

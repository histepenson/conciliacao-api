...
ConOut("ZFIN470API - [9b] filial_de=[" + cFilialDe + "] filial_ate=[" + cFilialAte + "] lAllFil=" + cValToChar(lAllFil))

If lAllFil .And. !Empty(cFilialDe) .And. !Empty(cFilialAte)
	// Range informado: filtra direto no SQL (nao depende de AdmGetFil, que so devolve
	// as filiais do contexto logado e nao alcanca filiais de outras unidades)
	cSqlWhere += " SE5.E5_FILIAL BETWEEN '" + PadR(cFilialDe, TamSX3("E5_FILIAL")[1]) + "' AND '" + PadR(cFilialAte, TamSX3("E5_FILIAL")[1]) + "' AND "
ElseIf lAllFil
